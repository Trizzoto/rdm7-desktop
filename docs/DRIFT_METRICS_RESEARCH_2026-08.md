...
| **3 · Error bar on every angle sample — 3/3** | Half the observed closure error of the leg between two straight-running anchors, spread linearly in time; 0.4°/s growth on open ends, capped at 8°, stamped 9° when there is no anchor at all. | **Corrected 2026-08-12** — "UDSM publishes nothing" was wrong. RDL carry `hea` and `hoa`, per-sample heading and horizontal accuracy, on every run their API serves (median 2.0° and 0.1 m). The claim survives only in its narrow form: they publish the *receiver's* accuracy, nobody publishes a confidence on the **derived drift angle** itself. DriftBox states a flat 1°, DriftMeter a flat ±0.5°. That is still a real differentiator, and a smaller one than this row used to claim. Note also that a system with measured heading barely needs a closure-error bar — ours exists because we reconstruct what they instrument. | Have it now | DriftBox datasheet: 1° on drift angle vs 0.1° on heading — the subtraction is what costs you |
...
| **20 · Run-to-run repeatability versus distance — 3/3** | Resample the angle and the turn rate onto a common course-distance axis; report RMS difference and correlation per corner and per run pair. | Repeatability is what actually wins events and almost nothing in the consumer market measures it. **The turn-rate version needs no closure and no anchors** — so it keeps working on exactly the sessions where the angle is refused. `gpArcLength` (:18380) and `gpSameSpotIn` (:19646) already exist. **Design the alignment to take two traces, not two laps:** RDL's chase "line/angle mimic score" is this same resample-and-RMS machinery pointed at two cars instead of two laps, so a tandem mimic number falls out of this metric for free if the interface is written that way from the start, and needs rewriting if it is not. | Worth new maths | FD's two-run qualifying rewards consistency directly |
...
| **25 · Directly measured body heading — 3/3** | True heading from the carrier-phase baseline between two GNSS antennas, differenced against Doppler course. | The architectural alternative and the ceiling everything else is measured against. It does not merely improve the angle: it **deletes** every rough leg, every no-anchor refusal, and every "this track has no straights" limitation at once. Confirmed by teardown: RDL's entire record is heading-measured, which is why it carries no closure machinery at all. Their median heading accuracy is 2.0° (min 0.4°, worst 18.1°) — **fused, not dual-antenna**, so a competitor judging Formula DRIFT is running roughly the accuracy class a good IMU fusion reaches, not the 0.1° a second antenna buys. | **Needs new hardware** — second antenna + moving-base receiver | VBOX 3i Dual Antenna <0.1° RMS at 1.0 m separation, <0.04° at 2.5 m, 100 Hz; u-blox ZED-F9H 0.4° static / 0.3° dynamic at 8-10 Hz; RDL/Wally ~2.0° median, measured off their API |
